...
public class AuthController {
    ...
    public ResponseEntity<String> register(@RequestBody UserRegistrationDTO userRegistrationDTO) {
        Optional<User> optionalUser = userService.findUserByUsername(userRegistrationDTO.username());
        if (optionalUser.isPresent())
            return new ResponseEntity<>("Username already exists", HttpStatusCode.valueOf(401));

        userService.createUser(userRegistrationDTO);
        return ResponseEntity.ok("Successfully Register the user: " + userRegistrationDTO.username());
    }
